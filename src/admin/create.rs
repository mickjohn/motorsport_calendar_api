...
fn derive_next_session_link(
    event_type: &str,
    new_session: &NewSession,
) -> Option<(NaiveDate, String)> {
    match event_type {
        "Formula 1" => match new_session.name.as_str() {
            "Practice 1" => new_session
                .date
                .map(|d| (d.date(), "Practice 2".to_string())),
            "Practice 2" => new_session
                .date
                .map(|d| (d.date().succ(), "Practice 3".to_string())),
            "Practice 3" => new_session
                .date
